...
public class JwtTokenService {
    ...
    private void validate(JwtProperties properties) {
        if (isBlank(properties.getIssuer())) {
            throw new IllegalStateException("Missing required configuration: tradeapp.jwt.issuer");
        }
        if (isBlank(properties.getSecret())) {
            throw new IllegalStateException("Missing required configuration: tradeapp.jwt.secret");
        }
        if (properties.getSecret().getBytes(StandardCharsets.UTF_8).length < 32) {
            throw new IllegalStateException(
                    "Configuration tradeapp.jwt.secret must be at least 32 bytes for HS256 signing");
        }
        if (Objects.requireNonNullElse(properties.getAccessTokenMinutes(), 0L) <= 0) {
            throw new IllegalStateException("Configuration tradeapp.jwt.access-token-minutes must be greater than zero");
        }
        if (Objects.requireNonNullElse(properties.getRefreshTokenDays(), 0L) <= 0) {
            throw new IllegalStateException("Configuration tradeapp.jwt.refresh-token-days must be greater than zero");
        }
    }
