...
import in.nishu.tradex.market_service.config.MarketHistoryProperties;
import in.nishu.tradex.market_service.entity.MarketPriceHistory;
import in.nishu.tradex.market_service.repositories.MarketPriceHistoryRepository;
...
public class HistoricalMarketDataSeeder implements SmartInitializingSingleton {
    ...
    private void seedMissingHistoryInTransaction() {
        ...
        private void seedMissingHistory(SupportedStockCatalog.StockSeed stock,String interval,LocalDateTime startTime,LocalDateTime endTime){
            ...
            BigDecimal previousClose=stock.refrencePrice().multiply(new BigDecimal("0.42").setScale(4, RoundingMode.HALF_UP));
        ...
        private GeneratedCandle nextCandle(SupportedStockCatalog.StockSeed stock,
                      LocalDate date, long dayIndex,
                      BigDecimal previosClose,Random random){
            ...
            BigDecimal open=previosClose.multiply(BigDecimal.valueOf(1+random.nextGaussian()*0.003))
                    .max(MIN_PRICE)
                    .setScale(4,RoundingMode.HALF_UP);
            BigDecimal close=previosClose.multiply(BigDecimal.valueOf(1+dailyReturn))
                    .max(MIN_PRICE)
                    .setScale(4,RoundingMode.HALF_UP);
            BigDecimal intradaySpread=previosClose.multiply(BigDecimal.valueOf(0.006+Math.abs(random.nextGaussian())*0.012))
                    .setScale(4,RoundingMode.HALF_UP);
        ...
        private  double crashFactor(LocalDate date){
            ...
            double inflationShock=eventMove(date,LocalDate.of(2022,1,25),LocalDate.of(2022,6,30),-0.0045);
        ...
        private long volume(SupportedStockCatalog.StockSeed stock,Random random,double absoluteReturn){
            long baseVolume=switch(stock.symbol()){
                ...
                case "HDFCBANK","ICICIBANK","AXIXBANK","BHARTIARTL"->3+200_000L;
                default ->1_300_000l;
            };
        ...
        private record GeneratedCandle(BigDecimal open,BigDecimal high,BigDecimal low,BigDecimal close,Long volume){
        }
}
